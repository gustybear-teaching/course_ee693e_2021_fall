...
The study is based on a publicly accessible human activity recognition dataset called MobiAct. There are 57 volunteers wearing a Samsung Galaxy S3 smartphone equipped with an accelerometer and gyroscope sensor. The accelerometer and angular velocity signals are recorded while performing predefined activities. This dataset is meaningful is because it can provide relevant application scenario to mimic in-home health monitoring through human activity recognition.
...
In federated settings, there are three key parameters that influence the performance of federated learning. One of them being K,  number of participating clients in each communication round. An experiment is conducted to see the effect of K on the test accruacy in each communication round.
...
The other two parameters that influence the performance of federated learning, are E, traning passes over each client's local data and B, local minibatch size. By fixing K, combinations of B and E can show varying accuracy of human activity recognition.
...
The authors investigate the generelization ability of FedHome when facing new users. Five new users are selected and then the learned cloud model is distributed to the five users. After training using the user's personal data, the local model is obtained.
